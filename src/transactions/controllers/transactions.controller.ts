...
export class TransactionsController {
    ...
    async findTxMe(@Param('account') account, @Res() response) {
        ...
        const accountFinder = await this.transactionsService.findTxMakeAccount(account);
        if (!accountFinder) {
            throw new HttpException({
    ...
    @Get('findMovements/:account/count')
    async findTxMeCount(@Param('account') account, @Res() response) {
        if (!account) { throw new HttpException({ status: HttpStatus.FORBIDDEN, error: 'missing Account' }, 403); }
        const accountFinder = await this.transactionsService.findTxMeCount(account);
        if (!accountFinder) {
            throw new HttpException({
                status: HttpStatus.BAD_REQUEST, error: 'Not found account',
            }, 400);
